...
import uk.gov.homeoffice.pontus.Log4j2TestBase;
...
public class Log4j2VanillaChronicleTest extends Log4j2TestBase {
    ...
    public void testVanillaAppender() throws IOException {
        ...
        String expected = PontusLog4j2Appender.uRunning.concat("Throwable test 1");
        evtMsg = evtMsg.substring(evtMsg.indexOf("Throwable"));
        expected = expected.substring(expected.indexOf("Throwable"));
        assertEquals(expected, evtMsg);
        ...
        evtMsg = evt.getMessage();
        expected = PontusLog4j2Appender.uRunning.concat("Throwable test 2");
        evtMsg = evtMsg.substring(evtMsg.indexOf("Throwable"));
        expected = expected.substring(expected.indexOf("Throwable"));
